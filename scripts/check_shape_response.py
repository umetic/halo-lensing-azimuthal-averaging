#!/usr/bin/env python3
"""Run tests through the centered-shape leading-response layer.

From the repository root, after activating the project environment:
    python scripts/check_shape_response.py

This validation does not run a full population field production and does not
render Figure 3. It tests the Figure-3 numerical analysis conventions using
compact targets and synthetic products.
"""
...
def main() -> int:
    ...
    parser.add_argument('--output-dir', default='outputs/shape_response_validation',
    ...
    metadata = {
        'validation_layer': 'shape-response-v0.1.0',
    ...
    print('Running tests through centered-shape leading-response analysis...')
    sys.stdout.flush()
    ...
    if result.returncode == 0:
        test_path = root/'tests/test_shape_response.py'
        spec = importlib.util.spec_from_file_location('_azlens_shape_response_test_helpers', test_path)
        if spec is not None and spec.loader is not None:
            helper = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(helper)
            from azlens.shape_response import centered_shape_response_rows, compact_shape_response_metrics, write_shape_response_csv
            rows = centered_shape_response_rows(helper._synthetic_shape_input(), n_bins=2)
            write_shape_response_csv(rows, output/'shape_response_smoke.csv')
            (output/'shape_response_smoke_metrics.json').write_text(
                json.dumps(compact_shape_response_metrics(rows), indent=2, sort_keys=True)+'\n', encoding='utf-8'
            )
            metadata['generated_products'] = ['shape_response_smoke.csv', 'shape_response_smoke_metrics.json']
